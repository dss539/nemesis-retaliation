...
Commit the post-review fixes as a second explicitly non-authoritative candidate, provision the gitignored source bytes in the isolated review worktrees, rerun all twelve specialties plus a holistic setup review on that exact commit, and disposition every result. Only a fully green rereview may become the v2 baseline and receive its immediate direct-child lock before source-only packets or sealed audit lanes begin. The population remains 56 concise rules + 28 base-applicable FAQ units + 56 stratified component effects.
...
- Branch `work/card-corpus-extraction` contains the non-authoritative candidate `5e525e12c105edb7565fa310ae16a526bc131025` over parent `3e23aebf52bac07ca54829fb49e4eb1f375bf599`; post-review fixes are pending a second candidate commit. No audit work has been pushed; no PR or deployment exists.
...
- Candidate-1 specialty/setup criticism is dispositioned in `docs/qa/implementation-readiness/correctness-audit/reviews/prelock-review-candidate-1-disposition.md`. Reproduced fixes cover lock creation, starting-HEAD ancestry, merge/full-history seals, POSIX paths, all 56 component targets, manifest-bound source evidence, raw-response binding, source-blocker precedence, exact progress refresh/history, wave seals, and isolated-worktree source provisioning. The interrupted source-only reviewer and provider-blocked mutation reviewer remain non-evidence and must be rerun.
...
- Fresh focused validation on 2026-08-30 is green: manifest freshness and prelock validation pass; all 73 mutation/provenance/progress controls and all 11 decision tests pass; source staging verifies 1,098 files. The complete neighboring source/status/diff checks will be rerun after this status update.
- A second exact-candidate twelve-specialty wave and completed holistic setup rereview are still required before the baseline/lock can be created. Provider-blocked, interrupted, or empty-content attempts remain non-evidence.
...
- Candidate 1 (`5e525e12`) is superseded as a review snapshot; its reproduced findings are fixed in the dirty canonical worktree and recorded in the candidate-1 disposition. Commit the exact second review candidate, provision gitignored sources with `scripts/stage_correctness_audit_sources.py`, rerun the twelve specialties plus holistic setup review, independently reproduce any new finding, and integrate only verified fixes. No candidate is the v2 baseline and no active lock may exist during review.
