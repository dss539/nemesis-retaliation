...
def build_robot_records(repo: Path, robot_source_index: dict, record, assertion, timing, participant, condition, decision, operation, exploration_rule_ids: list[str], room_rule_ids: list[str]) -> list[dict]:
    ...
    for card_id in sorted(ROBOT_DEFINITIONS):
        ...
        if card_id == 506400:
            ...
            add(sentence_id, "invoke-process", "if-able", "P-RULES", "one Serious Wound selected by the affected Character's player through the reusable discard/slide procedure", conditions=["discard-Wound branch selected"], decision_ref="D-RBT-529500-WOUND", target_ref="T-RBT-529500-WOUND", invoke="SEM-SERIOUS-WOUND-DISCARD-001")
