...
def build_intruder_help_records(repo, record, assertion, timing, participant, condition, operation):
    ...
    for side in source['sides']:
        ...
        row=side['bottomRow']
        occurrences[row['occurrenceId']]=row['printedInstruction']
        metadata[row['occurrenceId']]={'side':side['sideId'],'context':'all-token-draw-contexts'}
    ...
    trigger_by_suffix={
        ...
        'BOTTOM-01':'side-level Blank token front',
    }
    records=[]
    ...
    for occurrence_id in sorted(expected):
        ...
        lifecycle_assertion=assertion(f'SA-{occurrence_id}-LIFE','SRC-RULEBOOK','printed pages 15, 25, and 30 / token lifecycle, Corridor capacity, Surprise Attacks',['preconditions','operations','partialResolution','unresolvedQuestionRefs'],'After resolution, non-Blank tokens return to the matching pile and Blank returns to the bag unless printed text removes it. A Corridor holds 6 Intruder equivalents (Queen counts 4). An Intruder placed or moved into a Room with a Character immediately Attacks. The side-level Blank panel has no printed Corridor/Room/Bag scope label.','docs/rules/03-intruders-and-survival.md:INT-001')
        ...
        elif context=='bag-development':
            anchor='tax.process.temporal.phase.event'; context_taxa=['tax.scaffold.zone.intruder-bag']; timing_relation='during'
        else:
            anchor='tax.scaffold.zone.intruder-bag'; context_taxa=['tax.scaffold.zone.intruder-bag','tax.entity.spatial.corridor','tax.entity.spatial.room']; timing_relation='when-triggered'
        participants=[participant('P-RULES','rules-system')]
        trigger_key=occurrence_id.split('-',1)[1]
        context_predicate = f'caller context is {context}' if context != 'all-token-draw-contexts' else 'caller context is Corridor, Room, or Bag Development; exact add-Adults scope remains SEM-Q-099'
        preconditions=[condition('C-SIDE','predicate',[{'predicate':f'Intruder Help side is {side}'}],[f'SA-{occurrence_id}']),condition('C-CONTEXT','predicate',[{'predicate':context_predicate}],[f'SA-{occurrence_id}',f'SA-{occurrence_id}-LIFE'])]
        ...
        if occurrence_id in {'QA-C-01'}:
            ...
            ops=[operation('S01',1,'invoke-process','if-able','Queen','Activate Queen',[f'SA-{occurrence_id}'],conditions=['Queen activation is possible'],invoke='SEM-QUEEN-ACTIVATION-001'),operation('S02',2,'place-component','must','P-RULES','Queen in caller Corridor',[f'SA-{occurrence_id}'],conditions=['Queen activation is not possible'])]
        elif occurrence_id=='QA-R-01':
            term_refs=['term.queen']; taxon_refs+=['tax.entity.agent.intruder.queen']
            ops=[operation('S01',1,'invoke-process','if-able','Queen','Activate Queen',[f'SA-{occurrence_id}'],conditions=['Queen activation is possible'],invoke='SEM-QUEEN-ACTIVATION-001'),operation('S02',2,'place-component','must','P-RULES','Queen in caller Room',[f'SA-{occurrence_id}'],conditions=['Queen activation is not possible'])]
            ...
            ops=[operation('S01',1,'resolve-open-alternative','must','P-RULES','SEM-Q-105 mixed Adult/Drone allocation when capacity or exact-type model supply cannot satisfy the full token back',[f'SA-{occurrence_id}',f'SA-{occurrence_id}-LIFE'],conditions=['requested mixed counts exceed Corridor capacity or an exact-type model supply']),operation('S02',2,'place-component','if-able','P-RULES','exact requested Adults/Drones in caller Corridor from token-back counts',[f'SA-{occurrence_id}',f'SA-{occurrence_id}-LIFE'],repeat={'tokenFaceResolutions':face_resolution,'adultCount':'white number on token back','droneCount':'red addend on token back','corridorCapacityEquivalentLimit':6,'sourceVisualBasis':'numbered token backs show white Adult count and red Drone addend','allocationOrder':'SEM-Q-105 source-unspecified under scarcity/capacity'})]
            ...
            ops=[operation('S01',1,'resolve-open-alternative','must','P-RULES','SEM-Q-099 whether the side-level add-Adults instruction applies to Corridor/Room draws',[f'SA-{occurrence_id}',f'SA-{occurrence_id}-LIFE'],conditions=['caller context is Corridor or Room']),operation('S02',2,'draw-random','if-able','P-RULES','2 random Adult tokens from matching pile into Intruder bag',[f'SA-{occurrence_id}'],conditions=['caller context is Bag Development OR SEM-Q-099 universal-row alternative is adopted']),operation('S03',3,'transition-zone','must','P-RULES','drawn Blank token',[f'SA-{occurrence_id}',f'SA-{occurrence_id}-LIFE'],transition={'from':'drawn-token','to':'tax.scaffold.zone.intruder-bag'})]
        else:
        ...
        if context=='room' and occurrence_id in {'QA-R-01','QA-R-02','QD-R-01','QD-R-02'}:
            ...
            attack_op=operation('S99',99,'invoke-process','if-able','newly placed Intruder','Secure-entry/Attack resolution',[f'SA-{occurrence_id}-LIFE'],conditions=['this Help row newly placed an Intruder in caller Room','caller Room contains at least one Character'],invoke='SEM-SECURE-ENTRY-001')
        ...
        if occurrence_id in {'QA-C-02','QD-C-02'}:
            unresolved.append('SEM-Q-105')
        if occurrence_id in {'QA-BOTTOM-01','QD-BOTTOM-01'}:
            unresolved.append('SEM-Q-099')
        records.append(record(
            f'SEM-IH-{occurrence_id}',f'Intruder Help {occurrence_id}','source-backed-with-open-question' if unresolved else 'source-backed','dispatcher','official-primary','open-alternatives' if unresolved else 'verbatim-structure',
            ...
            {'policy':'source-limited-components','unit':'requested exact-type model/token','onImpossible':'use the general finite-component rule; SEM-Q-099/105 prohibit Blank-scope or mixed-type allocation defaults; resolve unaffected sentences'}, {'kind':'instantaneous-dispatch-row'}, {'policy':'one row per caller context and current Queen side'}, [], unresolved, []))
