...
def main() -> None:
    ...
    out_dir = ROOT / 'adjudication/contact-raw'
    log_dir = ROOT / 'logs/adjudication-contact'
    ...
    for ordinal, asset_id in enumerate(IDS, start=1):
        ...
        print(f'{asset_id} contact sealed session={session_id} api={ordinal}/8', flush=True)
    summary = {
        'schemaVersion': 1,
        'phase': 'post-blind-authoritative-contact-adjudication',
        'completedAt': now(),
    ...
    write_immutable(ROOT / 'metadata/postblind-contact-session.json', canonical(summary))
