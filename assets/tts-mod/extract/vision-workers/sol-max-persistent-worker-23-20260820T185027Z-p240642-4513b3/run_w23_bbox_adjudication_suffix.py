...
def main() -> None:
    ...
    out_dir = ROOT / 'adjudication/bbox-raw-retry-02'
    log_dir = ROOT / 'logs/adjudication-bbox-retry-02'
    ...
    for ordinal, asset_id in enumerate(IDS, start=1):
        ...
        print(f'{asset_id} bbox sealed session={session_id} api={ordinal}/7', flush=True)
    summary = {
        'schemaVersion': 1,
        'phase': 'post-blind-bbox-adjudication',
        'completedAt': now(),
    ...
    write_immutable(ROOT / 'metadata/postblind-bbox-session-segment-02.json', canonical(summary))
