...
- [~] Finish the deterministic v2 control layer without changing the seed or 140-unit population. Candidate 5 was rejected; the current post-disposition checkpoint preserves 56 concise rules + 28 FAQ units + 56 component effects, all pending, and is not yet independently reviewed.
...
- [ ] In a future bounded session, independently review the post-disposition checkpoint. Do not treat it as a baseline or create a v2 lock without usable acceptance and a repeated fully green gate.
- [ ] After acceptance, commit/promote the reviewed v2 baseline and create its audit lock as the immediate direct child containing only `audit-lock.json`, before any unit leaves `pending-blind-derivation`; no v2 lock exists yet.
