...
- [x] Restore a completely green focused prelock gate after candidate-1 findings. Verified 2026-08-30: manifest freshness and prelock validation pass; all 73 mutation/provenance/progress controls and all 11 decision tests pass; source staging verifies 1,098 files. Rerun source extraction, project status, and `git diff --check` before candidate 2.
- [~] Obtain and disposition a completed independent v2 setup review. Candidate-1 holistic review rejected the reproduced lock/evidence defects; those fixes are integrated. Rerun holistic review on candidate 2. Provider-blocked, interrupted, and empty-content attempts remain non-evidence.
