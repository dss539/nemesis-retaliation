...
- [~] Finish the deterministic v2 control-layer repair without changing the seed or 140-unit population. Current worktree state preserves 56 concise rules + 28 FAQ units + 56 component effects, all pending.
...
- [x] Restore a completely green focused prelock gate after candidate-3 findings. Verified 2026-08-30: manifest freshness and prelock validation pass; all 93 mutation/provenance/path/evidence/render/wave/progress/rollback controls and all 15 decision tests pass. Rerun source extraction, project status, source staging, and `git diff --check` before candidate 4.
- [~] Obtain and disposition a completed independent v2 setup review. Candidate-1 through candidate-3 specialty/setup findings are integrated and retained. Run targeted rereviews 1/3/5/7/10/11 plus holistic setup/lock review on candidate 4. Provider-blocked, interrupted, and empty-content attempts remain non-evidence.
- [ ] Commit the reviewed v2 baseline and create its direct-child audit lock before any unit leaves `pending-blind-derivation`; no v2 lock exists yet.
- [~] Parallel prelock review wave after a reproducible review-candidate commit; use twelve independent, read-only specialties in separate task workspaces/worktrees: (1) pre-seal/first-parent Git provenance, (2) baseline/lock supersession and immutability, (3) canonical path/symlink containment, (4) frozen hash-map collision and target binding, (5) strict JSON/schema fail-closed behavior, (6) reviewer identity and lane-order independence, (7) source-only prompt leakage and citation closure, (8) deterministic 23-unit clean-match selection, (9) evaluator thresholds and escalation, (10) progress transitions/rollback/HEAD binding, (11) mutation-control completeness and crash resistance, and (12) worktree-safe path resolution plus methodology/status consistency. Reproduce every alleged bypass locally before accepting it.
- [ ] Parallel Lane A packet-draft wave after the v2 lock: twenty-one isolated builders covering five concise-rule groups, two FAQ groups, and all fourteen component families. Builders prepare drafts concurrently; the integrator verifies every draft and seals the complete packet/prompt set in the shared packet wave commit.
- [ ] Parallel Lane A completeness-review wave: twenty-one reviewers different from the corresponding builders, using the same partition. Reject empty, malformed, interrupted, contradictory, or material-finding-bearing reviews as non-evidence; correct all accepted packet findings before sealing the complete review set in the shared completeness wave commit and beginning Lane B.
- [ ] Parallel Lane B blind-derivation wave: use the twenty-one source-family packet batches (within the authorized limit of 32). Give each worker only its accepted sealed packet, fixed instructions, and blind schema in an isolated non-repository directory; require every unit exactly once and in manifest order, then seal the complete blind set in the shared blind wave commit without retained reasoning traces.
- [ ] Parallel Lane C comparison wave: use up to 32 separate task workspaces/worktrees and different reviewer identities after all blind artifacts are sealed. Draft all 140 comparisons concurrently in balanced batches; the integrator verifies them and seals the complete Lane C set in the shared comparison wave commit before any adjudication.
- [ ] Parallel Lane D verification wave: after all comparisons are sealed, partition every non-match/blocker plus the deterministic 23 clean matches across up to 32 independent reviewers, maintaining identity separation. The integrator alone verifies completeness, advances progress, and runs the final evaluator.
...
- [ ] Worker security/operations discipline: one root flock per task workspace for the worker lifetime; no multiple workers below one workspace root; no profiles; no pipe-to-interpreter, heredoc, inline script, command built from model output, broad cleanup/reset/stash, PR, merge, deployment, implementation, or semantic expansion. Use file tools and saved scripts, and report exact paths, commits/hashes, checks, and caveats.
