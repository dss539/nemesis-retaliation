...
- [x] Restore a completely green focused prelock gate after candidate-2 findings. Verified 2026-08-30: manifest freshness and prelock validation pass; all 92 mutation/provenance/path/evidence/render/wave/progress/rollback controls and all 15 decision tests pass. Rerun source extraction, project status, source staging, and `git diff --check` before candidate 3.
- [~] Obtain and disposition a completed independent v2 setup review. Candidate-1 and candidate-2 specialty/setup findings are integrated and retained. Rerun all twelve specialties and the holistic review on candidate 3. Provider-blocked, interrupted, and empty-content attempts remain non-evidence.
