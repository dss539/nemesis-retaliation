...
- [x] Restore a completely green prelock gate through candidate 5. Verified 2026-08-31: manifest freshness and prelock validation pass; all 93 mutation/provenance/path/evidence/render/wave/progress/rollback controls and all 15 decision tests pass; source extraction, source staging, project status, and `git diff --check` pass.
- [~] Obtain and disposition candidate 5's targeted source-only inventory rereview and independent holistic setup/direct-child-lock lifecycle review. Candidate-1 through candidate-4 evidence and dispositions are retained; provider-blocked, interrupted, empty, malformed, or contradictory attempts remain non-evidence.
