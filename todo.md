...
- [ ] Owner decision on P1–P7. On approval: apply the cluster's consolidated edits in one repair commit, run the normal gate, single re-check per repaired ID against the freshly reread corpus, return passing fragments to the audit root without repair sidecars, one fresh verifier each.
