...
- [x] Restore a completely green prelock gate. Verified 2026-08-30: manifest freshness and prelock validation pass; all 61 mutation controls and all 10 decision tests pass; source extraction and project-status validation pass with zero failures; `git diff --check` is clean.
- [ ] Obtain and disposition a completed independent v2 setup review. Prior provider-blocked, interrupted, and empty-content attempts remain non-evidence.
...
- [ ] Parallel Lane A packet-draft wave after the v2 lock: twenty-one isolated builders covering five concise-rule groups, two FAQ groups, and all fourteen component families. Builders may prepare drafts concurrently, but the integrator must verify and seal them sequentially on the canonical first-parent chain.
- [ ] Parallel Lane A completeness-review wave: twenty-one reviewers different from the corresponding builders, using the same partition. Reject empty, malformed, interrupted, contradictory, or material-finding-bearing reviews as non-evidence; correct and reseal before Lane B.
- [ ] Parallel Lane B blind-derivation wave: partition the 140 units into up to 32 balanced, source-family-coherent batches. Give each worker only its accepted sealed packet, fixed instructions, and blind schema in an isolated non-repository directory; return each unit exactly once and in manifest order, without retained reasoning traces.
- [ ] Parallel Lane C comparison wave: use up to 32 separate task workspaces/worktrees and different reviewer identities after the corresponding blind artifacts are sealed. Draft comparisons concurrently; verify and seal them serially. All 140 comparison commits must precede every adjudication.
