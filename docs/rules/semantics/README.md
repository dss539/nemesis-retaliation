...
- `source-registry.json` — 412 exact source tuples, hashes, versions, authority, occurrence identities, and extraction-index links used by the corpus.
- `pilots.json` — 447 validated semantic records across 24 systems; the historical filename is retained while coverage expands.
- `review-gates.json` — eighty-three explicit open semantic questions with alternatives; no default is adopted.
- `contradictions.json` — sixty-nine source/semantic conflicts: fourteen authority-resolved, twenty-seven unresolved, and twenty-eight preserved boundaries.
- `coverage.json` — pilot scope and the large remaining base-game semantic surface.
- `backlog.json` — 600 source-obligation units across rule records, FAQ, visual obligations, Help sheets, and independent card/reference tuples; 443 are covered, 156 pending, and one source-blocked, with overlapping variants intentionally retained.
...
The backlog currently records 443 covered source obligations, 156 pending obligations, and one inherited exact-source blocker (`FACILITY RESTART`). The closed source-clear component batches now include the base competitive Objective/Mission family while retaining ten physically occluded official card occurrences, nine prototype/high-count physical exclusions, 18 generated selector gaps, and all Solo/Coop roots outside competitive dispatch. These are overlapping source-obligation counts, not a claim that 600 distinct game effects exist.
