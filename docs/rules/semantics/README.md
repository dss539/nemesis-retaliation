...
- `facility-source-index.json` — pinned source projection for the three-section Facility/map boundary, 21 rendered regular Room slots, 43 reserved paired connector gaps, two fixed border-piece named spaces, exact setup source segments, 19 selected visual obligations, finite inventory, and TTS map/setup provenance; geometry and actual Corridor occupancy remain separate.
- `source-registry.json` — 474 exact source tuples, hashes, versions, authority, occurrence identities, and extraction-index links used by the corpus.
- `pilots.json` — 529 validated semantic records across 26 systems; the historical filename is retained while coverage expands.
- `review-gates.json` — 115 explicit open semantic questions with alternatives; no default is adopted.
- `contradictions.json` — 88 source/semantic conflicts: 21 authority-resolved, 34 unresolved, and 33 preserved boundaries.
- `coverage.json` — pilot scope and the large remaining base-game semantic surface.
- `backlog.json` — 600 source-obligation units across rule records, FAQ, visual obligations, Help sheets, and independent card/reference tuples; 530 are covered, 69 pending, and one source-blocked, with overlapping variants intentionally retained.
...
Facility/map topology and setup-state batch: exact three-section rendered geometry, 21 regular variable Room slots with six edge directions and 43 reserved paired connector gaps, separate Landing Zone/Hibernatorium border-piece spaces, 23-tile/40-Corridor inventory, Room/Corridor/round-track/Character-board/marker setup state, 19 selected setup/map visual associations, five no-default setup questions, and six source-boundary conflicts; this remains partial base-game semantic coverage.
