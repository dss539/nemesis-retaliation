...
Extraction, controlled vocabulary, and static ontology gates are passed. The implementation-neutral semantic corpus is expanding by bounded source/component families; Event, Exploration, Robot, base Intruder Attack and Queen Health cards, Serious Wounds, regular Green/Red/Yellow Items, all 60 base Action cards, source-clear competitive Objective/Mission cards, Heavy/Equipment/Starting Items, and core Combat/Attacks/Noise/Hazard/entry procedures are closed at their current source boundaries. Unresolved timing, ownership, visibility, lifecycle, ordering, exhaustion, physical-class, occlusion, blocker, and variant questions remain explicit with no defaults. The next bounded work is source-clear Facility/map topology and setup-state semantics; implementation remains frozen.
