...
Extraction, controlled vocabulary, and static ontology gates are passed. The implementation-neutral semantic corpus is now expanding by bounded source/component families; Event, Exploration, Robot, and the 20-occurrence base Intruder Attack family are closed at their current source boundaries while unresolved timing, ownership, visibility, lifecycle, and variant questions remain explicit. Implementation is still frozen.
