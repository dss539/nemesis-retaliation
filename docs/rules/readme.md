...
Extraction, controlled vocabulary, and static ontology gates are passed. The implementation-neutral semantic corpus is now expanding by bounded source/component families; Event, Exploration, Robot, the 20-occurrence base Intruder Attack family, the 12-occurrence base Queen Health family, and the 27-occurrence base Serious Wound family are closed at their current source boundaries while unresolved timing, ownership, visibility, lifecycle, and variant questions remain explicit. The next bounded component batch should derive one base Item deck mechanically before encoding it, beginning with Green Items. Implementation is still frozen.
