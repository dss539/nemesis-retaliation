...
The owner approved the current atomic fragments and direct verdict method, then on 2026-09-04 approved self-scheduling work stealing for the remaining review. `audit/instructions.md` is the worker authority, `audit/protocol.md` is the parent authority, and `scripts/audit_work_steal.py` performs only the race-safe claim/finalize/recovery moves needed by that method. Do not rebuild the former Stage 1 harness or expand review into helper analysis. The 115 open questions and 88 preserved conflicts remain no-default. Solo/Coop and expansions remain outside rewrite readiness unless the owner changes scope.
