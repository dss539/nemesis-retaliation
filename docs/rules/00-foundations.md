...
- **Reinforced path check (Objective use):** The official `THE SUPPLY ROUTE` Mission Task note states that when checking its path from the Landing Zone to Life Support Control C, all Closed Doors and Intruders are ignored — they are irrelevant, only Reinforcement counts. **Source:** Official Objective Help Sheet unit `P1-MT-THE-SUPPLY-ROUTE` associated note (`OBJ-16`). The note is printed for this Mission Task only; whether the same reading applies to other path-based Objectives is not stated by any source. The TTS variant naming the Server Room as endpoint is a preserved source-variant difference recorded in INT-011.
