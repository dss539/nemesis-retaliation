...
- **Attack Panel Scope & Intermediate Death (Project Interpretation, SEM-Q-020 & SEM-Q-021):** Attack card effects (such as Fury or Claw operations) affect only Characters in the attacking Intruder's Room unless the card explicitly states Facility-wide scope. Attack panels resolve sequentially in printed order. If Health loss or a Serious Wound kills a targeted Character during the panel's resolution, that Character immediately stops participating; any subsequent Character-bound operations on that card targeting that Character do not resolve. If the card contains unaffected global/Room operations or targets surviving Characters, those resolve normally before the card is discarded.
...
- **Queen Final-Card Death Timing (Project Interpretation, SEM-Q-027):** If drawing the top Queen Health card empties the Queen Health deck, or if discarding the indicated additional cards exhausts the deck, resolve the bottom effect of the drawn card first. The Queen immediately dies upon completing the resolution of that card; remove the Queen model from the board.
...
| `assets/tts-mod/extract/v2-dl/tree/cards/game/objectiveMissonDeck-162_cards/card-18.png` | **OLD FRIEND:** “You must Escape from the Facility with at least 1 Corpse from the Hibernatorium.” | Evaluates via: INT-009, INT-011 (Escape definition), RT-009a; OQ-013: Excluded from base-game scope as expansion/add-on content (Gamefound add-on pack). |
...
- `corpses from the Hibernatorium`: resolved under OQ-013. The cards RETRIEVAL and OLD FRIEND reference components present only in the Gamefound add-on pack and absent from the official base-game rulebooks, component inventories, and help sheets; they are formally excluded from base-game scope. Affected IDs: `CARD-game-missionTaskDeck-game-missionTaskDeck-160_cards-card-05.png`, `CARD-game-objectiveMissonDeck-game-objectiveMissonDeck-162_cards-card-18.png`.
