...
def main():
 ...
 if aliases.get('counts')!={'aliases':8,'acceptedAliases':6,'proposedReviewAliases':2,'nonAliasBoundaries':5,'reviewGates':2}: failures.append({'check':'alias counts'})
 for a in alias_rows:
 ...
 # Two unresolved owner gates map exactly to the two proposed aliases.
 gates=review.get('gates') or []
 if review.get('counts')!={'reviewGates':2,'resolved':0,'open':2} or {g['reviewGateId'] for g in gates}!={'VG-001','VG-002'}: failures.append({'check':'review gates'})
 if {x for g in gates for x in g['affectedAliasIds']}!={a['aliasId'] for a in alias_rows if a['status'].startswith('proposed-')}: failures.append({'check':'review/alias linkage'})
